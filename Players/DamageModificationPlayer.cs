...
namespace OmniMod.Players{

    public class DamageModificationPlayer : ModPlayer{
		...
		public static void HandleExampleDodgeMessage(BinaryReader reader, int whoAmI) {
			...
			Main.player[player].GetModPlayer<DamageModificationPlayer>().ExampleDodgeEffects();
		...
		//	ModPacket packet = ModContent.GetInstance<OmniMod>().GetPacket();
		//	packet.Write((byte)OmniMod.MessageType.ExampleDodge);
